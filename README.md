...
flowchart LR
  ...
  h_capture -->|sets raw_b64 + overlays| h_raw
  h_capture -->|publish frame event| h_bus
  h_capture -->|awaits _ann_ready| h_ann
  h_capture -->|sets _frame_b64 + event.set| h_frame
  ...
  h_r_ann -->|sets _ann_result_b64 + ann_ready.set| h_ann
  ...
  b_harper -->|call_vlm_agent + image_url| h_call_agent
  b_benjamin -->|call_vlm_agent + image_url| h_call_agent
  b_lucas -->|call_vlm_agent + image_url| h_call_agent
  b_atlas -->|call_vlm_agent + image_url| h_call_agent
  b_nova -->|call_vlm_agent + image_url| h_call_agent
...
| Agent (Harper..Nova) | `max_agent_vlm_concurrent` | 1 |

Agent messages include the screen frame as `image_url` content — all 5 agents see the actual screen, not just Grok.

> **Note:** `max_agent_vlm_concurrent` defaults to `1` in `_async_main`. The `config.json` value `5` overrides this at runtime via `hub.cfg()`.
...
| `max_agent_vlm_concurrent` | `5` | Agent semaphore (config.json overrides code default of 1) |
...
All overlay dicts include `closed`, `stroke`, and `fill` keys.
